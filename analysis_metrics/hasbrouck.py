...
class InfoShares:
    ...
    def select_k_ar_diff(self, maxLag):
        res = vecm.select_order(
            self.y_t, maxlags=maxLag, deterministic=self.deterministic
        )
...
def mishares(corr, psi, omega):
    ...
    matrix_f_asterisk = np.dot(
        g.dot(np.power(np.linalg.inv(_lambda), 0.5)), g.transpose()
    ).dot(np.linalg.inv(v))
